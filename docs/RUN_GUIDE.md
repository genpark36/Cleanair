...
앱 설치와 센서 등록이 끝나면 Android 홈 화면에 Cleanair 위젯을 추가할 수 있습니다.

1. 휴대폰 홈 화면의 빈 공간을 길게 누릅니다.
2. “위젯” 메뉴를 엽니다.
3. Cleanair 항목에서 공기질 요약 위젯 또는 추세 위젯을 선택합니다.
4. 홈 화면에 배치합니다.

위젯은 앱에서 선택한 센서와 저장된 위치를 기준으로 표시합니다. 값이 보이지 않으면 앱을 한 번 열어 센서가 정상적으로 연결되어 있는지 확인하세요. 위젯을 누르면 Cleanair 앱이 열립니다.
