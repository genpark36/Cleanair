...
document.addEventListener("click", (event) => {
  ...
  if (actionName === "end-incident") endSituation({ incidentId, alertId });
...
function renderCriticalBanner() {
  ...
  const critical = incident || getCriticalAlert();
  if (!critical) {
  ...
  const endTargetAttr = incident
    ? `data-incident-id="${escapeAttr(incident.id)}"`
    : `data-alert-id="${escapeAttr(critical.id)}"`;
  ...
  els.criticalBanner.innerHTML = `
    ...
    <div class="critical-actions">
      ...
      ${critical.id ? `<button class="white-button" data-action="end-incident" ${endTargetAttr}>상황 종료</button>` : ""}
    </div>
  `;
}
...
function renderAlerts() {
  const items = [
    ...
    ...state.alerts.filter((alert) => !alert.isResolved),
  ].slice(0, 4);
  if (!items.length) {
...
function openEmergencyModal() {
  ...
  const endButton = incident
    ? `<button class="soft-button" data-action="end-incident" data-incident-id="${escapeAttr(incident.id)}">상황 종료</button>`
    : alert?.id
      ? `<button class="soft-button" data-action="end-incident" data-alert-id="${escapeAttr(alert.id)}">상황 종료</button>`
      : "";
...
async function endSituation({ incidentId, alertId } = {}) {
  if (incidentId) {
...
async function endAlert(alertId) {
  if (!alertId) return;
  try {
    await callFunction("resolveAlert", {
      alertId,
      resolvedBy: state.user?.email || state.user?.uid || "dashboard",
      resolutionNote: "closed_from_dashboard",
    });
...
function buildTimeline() {
  ...
  const alertRows = state.alerts.slice(0, 30).map((alert) => ({
    title: alert.isResolved ? `${alert.title} 종료` : alert.title,
    message: alert.isResolved
      ? `${formatTime(alert.resolvedAt)}에 종료 처리되었습니다.`
      : alert.message || alert.location || "상세 내용 없음",
    time: alert.isResolved ? alert.resolvedAt || alert.updatedAt || alert.createdAt : alert.createdAt,
    critical: alert.isCritical && !alert.isResolved,
  }));
...
function normalizeAlert(id, data = {}) {
  ...
  const resolvedAt = firstTime(data.resolvedAt, data.closedAt);
  const isResolved = ["resolved", "closed", "ended", "archived"].includes(status)
    || Boolean(resolvedAt);
  ...
  return {
    ...
    resolvedAt,
    resolvedBy: firstText(data.resolvedBy, data.closedBy, ""),
    isResolved,
...
function isVisibleAlert(alert) {
  ...
  if (alert.isResolved) return hasRecentUpdate(alert.resolvedAt || alert.updatedAt || alert.createdAt, 30);
...
function getCriticalAlert() {
  return state.alerts.find((alert) => alert.isCritical && !alert.isResolved) || null;
}
