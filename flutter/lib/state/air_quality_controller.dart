...
class AirQualityController extends ChangeNotifier {
  ...
  PurificationSummary? get purificationSnapshot =>
      _latestSnapshot?.purification;
  ...
  Future<void> initialize() async {
    ...
    await refreshHistoryFromFirestore();
  ...
  Future<void> refreshHistoryFromFirestore() async {
    await _loadFirestoreHistory();
  }

  Future<bool> refreshExternalComparison() async {
    final current = _latestSnapshot;
    if (current == null) return false;

    final comparison = await _service.refreshExternalComparison(current);
    if (comparison == null) return false;

    final updated = current.copyWith(locationComparison: comparison);
    _latestSnapshot = updated;
    final updatedMs = updated.timestamp.millisecondsSinceEpoch;
    final index = _history.indexWhere(
      (entry) => entry.timestamp.millisecondsSinceEpoch == updatedMs,
    );
    if (index >= 0) {
  ...
  Future<void> applyLocalSnapshot(AirQualitySnapshot snapshot) async {
    _lastError = null;
  ...
  Future<void> _loadFirestoreHistory() async {
    try {
      ...
      if (_history.isNotEmpty) {
        since = _history.first.timestamp.subtract(const Duration(hours: 1));
      } else {
        since = DateTime.now().subtract(_historyWindow);
      }

      final firestoreHistory = await _service.loadHistory(
        since: since,
        limit: _maxHistoryEntries,
      );
      if (firestoreHistory.isEmpty) return;

      final existingTimestamps = <int>{
        for (final snapshot in _history)
          snapshot.timestamp.millisecondsSinceEpoch,
      };
      var added = 0;
      for (final snapshot in firestoreHistory) {
        final ms = snapshot.timestamp.millisecondsSinceEpoch;
        if (existingTimestamps.contains(ms)) continue;
        _history.add(snapshot);
        existingTimestamps.add(ms);
        added++;
      }
      ...
      try {
        // ignore: avoid_print
        print('[debug] Firestore history load failed: $error');
      } catch (_) {}
    }
  }
  ...
  Future<void> _connectInternal({bool force = false}) async {
    ...
    try {
      ...
      unawaited(refreshHistoryFromFirestore());
    } catch (error) {
  ...
  void _handleSnapshot(AirQualitySnapshot snapshot) {
    ...
    if (!_hasMeasurementValues(snapshot)) {
      _history.clear();
  ...
  bool _hasMeasurementValues(AirQualitySnapshot snapshot) {
