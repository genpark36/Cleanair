...
class AirQualitySnapshot {
  ...
  factory AirQualitySnapshot.fromJson(Map<String, dynamic> json) {
    ...
    return AirQualitySnapshot(
      ...
      co: _asDouble(raw?['co']) ??
          _asDouble(raw?['co_ppm']) ??
          _asDouble(raw?['carbon_monoxide']) ??
          _asDouble(raw?['carbonMonoxide']) ??
          _asDouble(json['co']) ??
          _asDouble(json['co_ppm']) ??
          _asDouble(json['carbon_monoxide']) ??
          _asDouble(json['carbonMonoxide']),
      ...
      iaqiScore: _asDouble(derived?['iaqiScore']) ??
          _asDouble(derived?['m_score']) ??
          _asDouble(derived?['mScore']) ??
          _asDouble(raw?['iaqiScore']),
      aqiLevel: derived?['aqiLevel']?.toString() ??
          derived?['primary_grade']?.toString(),
      aqiCategory: derived?['aqiCategory']?.toString() ??
          derived?['sub_level']?.toString() ??
          derived?['primary_grade']?.toString(),
...
class LocationComparisonSnapshot {
  ...
  static LocationComparisonSnapshot? tryParse(Map<String, dynamic>? json) {
    ...
    return LocationComparisonSnapshot(
      ...
      stationLatitude: _asDouble(json['stationLatitude']),
      stationLongitude: _asDouble(json['stationLongitude']),
...
class SeniorHealthSnapshot {
  ...
  static SeniorHealthSnapshot? tryParse(Map<String, dynamic>? json) {
    ...
    return SeniorHealthSnapshot(
      ...
      pmExposure: SeniorPmExposureSnapshot.tryParse(_asMap(json['pmExposure'])),
      ventilation:
...
class PurificationCadrSnapshot {
  ...
  // New dual-k analysis fields
  final String?
      mode; // 'Purifier', 'Ventilation', 'Combined', 'Stagnant', 'Accumulating'
  final String? dominantSource; // 'PM', 'CO2', 'Both', 'Neither'
  final String?
      scenario; // e.g., 'PM Rapid Decay (Sealed)', 'CO2 Rapid Decay (Ventilation)'
  ...
  static PurificationCadrSnapshot? tryParse(Map<String, dynamic>? json) {
    ...
    return PurificationCadrSnapshot(
      ...
      pmAccuracyWarning:
          (_asDouble(json['r2_pm25'] ?? json['r2Pm25']) ?? 1.0) < 0.7,
      co2AccuracyWarning:
          (_asDouble(json['r2_co2'] ?? json['r2Co2']) ?? 1.0) < 0.7,
    );
  }
}
