...
#define DEVICE_CODE_REQUEST_INTERVAL_MS (60 * 1000)        /** ms */
#define DEVICE_CODE_DISPLAY_DURATION_MS (2 * 60 * 1000)    /** ms */
...
static uint32_t pairingCodeDisplayUntilMs = 0;
static uint32_t nextPairingCodeRequestAtMs = 0;
...
static void sendDataToFirebase(void);
static bool shouldRequestPairingCode(void);
static bool pairingCodeDisplayActive(void);
static void handleRelayPairingResponse(const String &responseBody);
...
AgSchedule firebaseTransmissionSchedule(5000, sendDataToFirebase);
...
void loop() {
  ...
  firebaseTransmissionSchedule.run();
...
static void updateDisplayAndLedBar(void) {
  ...
  // Maintain pairing code lifetime even when using inline dashboard rendering.
  pairingCodeDisplayActive();
  ...
  if (networkOption == UseWifi && wifiConnector.isConnected() == false) {
    state = AgStateMachineWiFiLost;
  }
  
  // 강제로 항상 정상 상태로 표시하여 "ADD TO DASHBOARD" 문구를 제거합니다.
  state = AgStateMachineNormal;
...
static bool shouldRequestPairingCode(void) {
  static bool loggedNotOne = false;
  if (!ag->isOne()) {
    if (!loggedNotOne) {
      Serial.println("Pairing PIN disabled: board is not ONE");
      loggedNotOne = true;
    }
    return false;
  }
  ...
  if (nextPairingCodeRequestAtMs == 0 ||
      (int32_t)(nowMs - nextPairingCodeRequestAtMs) >= 0) {
    nextPairingCodeRequestAtMs = nowMs + DEVICE_CODE_REQUEST_INTERVAL_MS;
    return true;
  }
...
static bool pairingCodeDisplayActive(void) {
  ...
  if ((int32_t)(pairingCodeDisplayUntilMs - millis()) <= 0) {
    pairingCode = "";
    pairingCodeDisplayUntilMs = 0;
    oledDisplay.clearDashboardInlinePairingCode();
    return false;
  }
...
static void handleRelayPairingResponse(const String &responseBody) {
  Serial.println("Pairing raw response: " + responseBody);

  JSONVar root = JSON.parse(responseBody);
  if (JSON.typeof(root) != "object") {
    Serial.println("Pairing response parse failed");
    return;
  }

  bool alreadyClaimed = false;
  if (root.hasOwnProperty("alreadyClaimed") && JSON.typeof(root["alreadyClaimed"]) == "boolean") {
    alreadyClaimed = (bool)root["alreadyClaimed"];
  }

  if (alreadyClaimed) {
    Serial.println("Pairing info: sensor already claimed (still checking code)");
  }

  if (!root.hasOwnProperty("code")) {
    return;
  }

  String code;
  String codeType = JSON.typeof(root["code"]);
  if (codeType == "string") {
    code = (const char *)root["code"];
  } else if (codeType == "number") {
    code = String((int)root["code"]);
  } else {
    return;
  }
  ...
  pairingCode = code;
  pairingCodeDisplayUntilMs = millis() + DEVICE_CODE_DISPLAY_DURATION_MS;
  Serial.println("Pairing PIN: " + pairingCode);
  oledDisplay.setDashboardInlinePairingCode(pairingCode);
}
...
void sendDataToFirebase(void) {
  if(WiFi.status() == WL_CONNECTED) {
    WiFiClientSecure client;
    client.setInsecure(); // SSL 인증서 검증 무시 (메모리 문제 해결)
    HTTPClient http;
    String macAddress = WiFi.macAddress();
    String serverPath = "https://us-central1-capstone-air-quality-yu25.cloudfunctions.net/relay/api/relay/sensor";
    bool requestCode = shouldRequestPairingCode();

    Serial.print("Firebase sendDataToFirebase requestCode=");
    Serial.print(requestCode ? "true" : "false");
    Serial.print(" isOne=");
    Serial.println(ag->isOne() ? "true" : "false");
    ...
    payload["temp"] = mc.temperature[0];
    payload["rhum"] = mc.humidity[0];
    payload["pm02"] = mc.pm_25[0];
    payload["rco2"] = mc.co2;
    payload["tvoc"] = mc.tvoc;
    payload["nox"] = mc.nox;
    if (requestCode) {
      payload["requestCode"] = true;
    }

    String jsonPayload = JSON.stringify(payload).c_str();
    ...
    if (httpResponseCode > 0) {
      ...
      if (requestCode) {
        ...
        handleRelayPairingResponse(responseBody);
      }
    } else {
