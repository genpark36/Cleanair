...
void OledDisplay::showTempHum(bool hasStatus) {
  ...
  // Keep pairing PIN visible without replacing the main dashboard layout.
  if (dashboardInlinePairingCode.length() == 6) {
    // Use a smaller font and lock the code inside the gap between temp and humidity.
    DISP()->setFont(u8g2_font_t0_12_tf);
    const int centerLeftX = 48;
    const int centerRightX = 102;
    const int centerWidth = centerRightX - centerLeftX;
    int codeWidth = DISP()->getStrWidth(dashboardInlinePairingCode.c_str());
    int codeX = centerLeftX + ((centerWidth - codeWidth) / 2);

    if (codeX < centerLeftX) {
      codeX = centerLeftX;
    }
    if ((codeX + codeWidth) > centerRightX) {
      codeX = centerRightX - codeWidth;
    }
    if (codeX < centerLeftX) {
      codeX = centerLeftX;
    }

    DISP()->drawStr(codeX, 11, dashboardInlinePairingCode.c_str());
...
void OledDisplay::setDashboardInlinePairingCode(const String &code) {
  if (code.length() == 6) {
    dashboardInlinePairingCode = code;
  }
}

void OledDisplay::clearDashboardInlinePairingCode(void) {
  dashboardInlinePairingCode = "";
}
